...
<!-- IMAGE_START: _pictures_0.png | label=Figure 1 | caption=Technology tree of RAG research. The stages of involving RAG mainly include pre-training, fine-tuning, and inference. With the emergence of LLMs, research on RAG initially focused on leveraging the powerful in context learning abilities of LLMs, primarily concentrating on the inference stage. Subsequent research has delved deeper, gradually integrating more with the fine-tuning of LLMs. Researchers have also been exploring ways to enhance language models in the pre-training stage through retrieval-augmented techniques. -->
**Figure 1** (Fig. I): Technology tree of RAG research. The stages of involving RAG mainly include pre-training, fine-tuning, and inference. With the emergence of LLMs, research on RAG initially focused on leveraging the powerful in context learning abilities of LLMs, primarily concentrating on the inference stage. Subsequent research has delved deeper, gradually integrating more with the fine-tuning of LLMs. Researchers have also been exploring ways to enhance language models in the pre-training stage through retrieval-augmented techniques.
This image is a detailed, timeline-based infographic that visualizes the evolution and categorization of techniques within "Retrieval—Augmented Generation" (RAG). It uses a tree-like structure, with a central root at the bottom branching upwards, signifying the progression of research and development over time. The background is white.

**1. Overall Title:**
*   At the very bottom center, forming the root of the tree, is the main title: "Retrieval—Augmented Generation" (in dark green text).

**2. Timeline (Left Vertical Axis):**
*   A vertical gray line on the left serves as the timeline, marked with specific years and major language model milestones. Horizontal dashed gray lines extend from these markers across the diagram.
    *   **2020** (bottom, corresponding to the initial RAG developments)
    *   **GPT-3** (positioned slightly above "2020")
    *   **2023** (central, broadly aligned with the emergence of "ChatGPT")
    *   **ChatGPT** (positioned slightly below "2023")
    *   **GPT-4** (positioned above "2023")
    *   **2024** (at the top of the timeline)
*   The text for these timeline markers (2020, GPT-3, 2023, ChatGPT, GPT-4, 2024) is in a pinkish-red color.

**3. Legend (Bottom Right):**
*   A rectangular box in the bottom-right corner provides a legend titled "Augmentation Stage." It defines the color coding used for the three main branches of the tree:
    *   **Fine-tuning:** Green
    *   **Pre-training:** Orange
    *   **Inference:** Blue

**4. Branches and Associated Techniques:**
The tree divides into three main branches, each colored according to the "Augmentation Stage" and populated with oval-shaped nodes, each containing the name of a specific RAG technique. The vertical position of these nodes indicates their approximate time of emergence, from older (bottom) to newer (top).

**A. Fine-tuning Branch (Green):**
*   **Stage Title:** "Fine-tuning" (at the top of this green branch).
*   **Techniques (listed from approximately oldest to newest):**
    *   **Around 2020 (near GPT-3):**
        *   RAG
        *   PROMPTTAGATOR
        *   RAG-e2e
        *   Zemi
        *   Atlas
        *   R-GQA
    *   **Between 2020 and 2023 (lower-mid section):**
        *   SUGRE
        *   AAR
        *   FLARE
        *   UPRISE
    ...
    *   **Around 2023 (near ChatGPT):**
        ...
        *   EAR
        *   CoN
    *   **Around 2024 (top-most):**
        *   UniMS-RAG
    *   *(Total Fine-tuning techniques: 26)*

**B. Pre-training Branch (Orange):**
*   **Stage Title:** "Pre-training" (at the top of this orange branch).
*   **Techniques (listed from approximately oldest to newest):**
    *   **Between 2020 and 2023 (lower-mid section):**
        *   TIGER
        *   RAVEN
    *   **Around 2023 (near ChatGPT):**
        ...
        *   CoG
        *   InstructRetro
    *   *(Total Pre-training techniques: 5)*

**C. Inference Branch (Blue):**
*   **Stage Title:** "Inference" (at the top of this blue branch).
*   **Techniques (listed from approximately oldest to newest):**
    *   **Around 2020 (near GPT-3):**
        *   RECITE
        *   GenRead
    ...
    *   **Between 2020 and 2023 (lower-mid section):**
        ...
        *   IRCOT
        *   KnowledGPT
    *   **Around 2023 (near ChatGPT) and extending towards 2024:**
        *   ITRG
        *   KALMV
        *   Token-Elimination
        *   PRCA
...
<!-- IMAGE_START: _pictures_1.png | label=Figure 2 | caption=A representative instance of the RAG process applied to question answering. It mainly consists of 3 steps. 1) Indexing. Documents are split into chunks, encoded into vectors, and stored in a vector database. 2) Retrieval. Retrieve the Top k chunks most relevant to the question based on semantic similarity. 3) Generation. Input the original question and the retrieved chunks together into LLM to generate the final answer. -->
**Figure 2** (Fig. II): A representative instance of the RAG process applied to question answering. It mainly consists of 3 steps. 1) Indexing. Documents are split into chunks, encoded into vectors, and stored in a vector database. 2) Retrieval. Retrieve the Top k chunks most relevant to the question based on semantic similarity. 3) Generation. Input the original question and the retrieved chunks together into LLM to generate the final answer.
This image is a flowchart illustrating the process of Retrieval Augmented Generation (RAG) in a Large Language Model (LLM) system, contrasting it with a system without RAG.

The diagram is structured into several interconnected blocks:

**1. Top Left: User Input and Output**
*   **User** (blue rounded rectangle): Represents the human user interacting with the system.
    *   An arrow labeled "Input" points from "User" to the "Query" box.
    *   An arrow labeled "Output" points from the "Answer" box back to "User."

**2. Top Middle: Input Block**
*   **Input** (pale yellow rounded rectangle with dashed border)
    *   Contains a nested blue rounded rectangle labeled **Query**.
    *   **Query Text:** "How do you evaluate the fact that OpenAI's CEO, Sam Altman, went through a sudden dismissal by the board in just three days, and then was rehired by the company, resembling a real-life version of "Game of Thrones" in terms of power dynamics?"

**3. Top Right: Indexing Block**
*   **Indexing** (pale purple rounded rectangle with dashed border)
    *   Contains an icon representing three stacked documents.
    *   **Documents** (label below the document icon): An arrow points from "Documents" to a stack of horizontal bars icon.
    *   **Chunks/Vectors** (label below the stack icon): This indicates the result of processing documents.
    *   An arrow points from "Documents" downwards to "Relevant Documents" in the "Retrieval" block.
    *   An arrow points from "Chunks/Vectors" downwards to "embeddings" (blue rounded rectangle).
    *   An arrow points from "embeddings" downwards to "Relevant Documents."

**4. Middle Right: Retrieval Block**
*   **Retrieval** (pale pink rounded rectangle with dashed border)
    *   Contains a nested blue rounded rectangle labeled **Relevant Documents**.
    *   Below "Relevant Documents" are three listed text chunks, each prefixed with "Chunk X:".
    ...
    *   An arrow points from "Relevant Documents" to "Combine Context and Prompts" in the "Generation" block.

**5. Bottom Middle: Generation Block**
*   **Generation** (pale gray rounded rectangle with dashed border)
    *   Contains a central label: **LLM** (blue snowflake icon next to text).
    *   An arrow points from the "Query" (Input block) to "LLM".
    *   Below "LLM" is a text area with a dashed border, representing the prompt construction.
        *   **Question :**
        *   "How do you evaluate the fact that the OpenAI's CEO, .... dynamics?" (Truncated version of the original query)
        *   **Please answer the above questions based on the following information :**
        *   "Chunk 1 :"
        *   "Chunk 2 :"
        *   "Chunk 3 :"
    *   Below this text area is a blue rounded rectangle labeled **Combine Context and Prompts**.
    *   An arrow points from "Combine Context and Prompts" to "LLM".
    *   An arrow points from "LLM" to the "Answer" block.

**6. Bottom Left: Answer Block**
*   **Answer** (blue rounded rectangle)
    *   Above "Answer" are two distinct answer boxes, illustrating the difference between "without RAG" and "with RAG."

    *   **Top Answer Box (without RAG):**
        *   Labeled **without RAG** (light gray rounded rectangle with a sad face emoji).
        *   Includes an "X" icon in the top right, indicating a less satisfactory or incomplete answer.
        *   **Answer Text:** "...I am unable to provide comments on future events. Currently, I do not have any information regarding the dismissal and rehiring of OpenAI's CEO ..."
        *   An arrow points from "LLM" directly to this "without RAG" answer box.

    *   **Bottom Answer Box (with RAG):**
        *   Labeled **with RAG** (light orange rounded rectangle with a happy face emoji).
        *   Includes a checkmark icon in the top right, indicating a better or more complete answer.
        *   **Answer Text:** "...This suggests significant internal disagreements within OpenAI regarding the company's future direction and strategic decisions. All of these twists and turns reflect power struggles and corporate governance issues within OpenAI..."
        *   An arrow points from "LLM" to this "with RAG" answer box, specifically from the process after "Combine Context and Prompts."

**Flow of Information:**

1.  **User Input:** The "User" provides a "Query."
2.  **RAG Path (with context):**
    *   The "Query" is sent to the "LLM" (Generation block).
    *   Simultaneously, "Documents" are fed into "Indexing" to create "Chunks/Vectors" and "embeddings."
    *   The "Query" also triggers "Retrieval" of "Relevant Documents" using the "Documents" and "embeddings."
    *   The "Relevant Documents" (Chunk 1, Chunk 2, Chunk 3) are combined with the original "Query" in "Combine Context and Prompts" to form an enriched prompt.
    *   This enriched prompt is sent to the "LLM."
    *   The "LLM" then generates the detailed "with RAG" "Answer."
3.  **Non-RAG Path (without context):**
    *   The "Query" is sent directly to the "LLM" without any external relevant documents or context.
    *   The "LLM" generates the generic "without RAG" "Answer," stating it lacks information.
<!-- IMAGE_END -->
...
<!-- IMAGE_START: _pictures_2.png | label=Figure 3 | caption=Comparison between the three paradigms of RAG. (Left) Naive RAG mainly consists of three parts: indexing, retrieval and generation. (Middle) Advanced RAG proposes multiple optimization strategies around pre-retrieval and post-retrieval, with a process similar to the Naive RAG, still following a chain-like structure. (Right) Modular RAG inherits and develops from the previous paradigm, showcasing greater flexibility overall. This is evident in the introduction of multiple specific functional modules and the replacement of existing modules. The overall process is not limited to sequential retrieval and generation; it includes methods such as iterative and adaptive retrieval. -->
**Figure 3** (Fig. III): Comparison between the three paradigms of RAG. (Left) Naive RAG mainly consists of three parts: indexing, retrieval and generation. (Middle) Advanced RAG proposes multiple optimization strategies around pre-retrieval and post-retrieval, with a process similar to the Naive RAG, still following a chain-like structure. (Right) Modular RAG inherits and develops from the previous paradigm, showcasing greater flexibility overall. This is evident in the introduction of multiple specific functional modules and the replacement of existing modules. The overall process is not limited to sequential retrieval and generation; it includes methods such as iterative and adaptive retrieval.
This image presents a comparative overview of different Retrieval Augmented Generation (RAG) architectures, categorized into "Naive RAG," "Advanced RAG," and "Modular RAG." The image is structured into three main vertical sections, each illustrating a distinct RAG approach with its corresponding title at the bottom.
...
*   **Title:** "Naive RAG"
*   **Description:** This section depicts a straightforward, linear RAG process flow using solid blue arrows to indicate progression.
    *   **Input Layer:** At the very top, there are two distinct input sources:
        *   "User" (represented by a person icon) and "Query" (represented by a document with horizontal lines icon).
        *   "Documents" (represented by a chain link icon and a stack of three document icons).
    *   **Indexing Step:** The "Documents" input flows into an "Indexing" module (a rectangular box).
    *   **Retrieval Step:** Both the "User/Query" input and the output from "Indexing" converge into a "Retrieval" module (a rectangular box).
    *   **Generation Step:** The output from "Retrieval" feeds into a "Prompt" (a document with horizontal lines icon) which is then directed towards a "Frozen LLM" (represented by a snowflake icon). An arrow explicitly points from "Prompt" to "Frozen LLM".
    *   **Output Layer:** The combined output from "Prompt" and "Frozen LLM" results in the final "Output" (a rectangular box).

---

**Section 2: Advanced RAG**

*   **Title:** "Advanced RAG"
*   **Description:** This section illustrates an enhanced RAG process, introducing "Pre-Retrieval" and "Post-Retrieval" stages, indicated primarily by orange elements and arrows.
    *   **Input Layer:** Similar to Naive RAG, it starts with:
        *   "User" (person icon) and "Query" (document icon).
        *   "Documents" (chain link and stack of documents icons).
    *   **Indexing Step:** The "Documents" input flows into an "Indexing" module (a rectangular box), identical to Naive RAG. (Blue arrow).
    *   **Pre-Retrieval Step:** The "User/Query" input first goes through a "Pre-Retrieval" module (an orange rectangular box). This module is detailed with a bulleted list of operations:
        *   "Query Routing"
        *   "Query Rewriting"
        *   "Query Expansion"
        *   (Orange arrow from User/Query to Pre-Retrieval).
    *   **Retrieval Step:** The output from "Pre-Retrieval" (orange arrow) and the output from "Indexing" (blue arrow) both feed into the "Retrieval" module (a rectangular box).
    *   **Post-Retrieval Step:** The output from "Retrieval" then proceeds to a "Post-Retrieval" module (an orange rectangular box). This module includes three distinct functionalities, each with an icon:
        *   "Rerank" (two curved arrows pointing away from each other).
        *   "Summary" (an audio waveform icon with arrows indicating compression/expansion).
        *   "Fusion" (a stack of three trapezoidal shapes).
        *   (Orange arrow from Retrieval to Post-Retrieval).
    *   **Generation Step:** The output from "Post-Retrieval" feeds into a "Prompt" (document icon) which is directed towards a "Frozen LLM" (snowflake icon). An arrow points from "Prompt" to "Frozen LLM". (Orange arrow from Post-Retrieval to Prompt).
    *   **Output Layer:** The combined output from "Prompt" and "Frozen LLM" leads to the final "Output" (a rectangular box).

---

**Section 3: Modular RAG**

*   **Title:** "Modular RAG"
*   **Description:** This section is divided into two sub-sections: "Modules" and "Patterns", illustrating a more flexible and component-based approach to RAG.

    *   **Sub-section A: Modules**
        *   **Title:** "Modules"
        *   **Description:** A network diagram showing various interconnected RAG components. Components are represented by rounded rectangles or ovals with dashed lines indicating connections.
        *   **Central RAG Operations (Orange Ovals):**
            *   "Retrieve" (top-center)
            *   "Read" (bottom-center)
            *   A central label "RAG" is positioned between "Retrieve" and "Read" but is not a module itself.
        *   **Supporting Modules (Blue Ovals/Rectangles):**
            *   "Routing" (top-left)
            *   "Search" (top-right of Routing, above Retrieve)
            *   "Predict" (top-right)
            *   "Rewrite" (mid-left)
            *   "Rerank" (mid-right)
            *   "Demonstrate" (bottom-left)
            *   "Memory" (bottom-center, below Read)
            *   "Fusion" (bottom-right)
        *   **Interconnections (Dashed Lines):** All modules are interconnected, showing a flexible architecture. For example:
            *   "Routing" connects to "Search", "Rewrite", "Demonstrate".
            *   "Retrieve" connects to "Search", "Rewrite", "Rerank", "Read".
            *   "Read" connects to "Retrieve", "Rerank", "Memory", "Predict".
            *   "Memory" connects to "Rewrite", "Demonstrate", "Read", "Fusion", "Predict".
            *   Many other connections exist between all modules.

    *   **Sub-section B: Patterns**
        *   **Title:** "Patterns"
        *   **Description:** This sub-section presents four distinct RAG workflow patterns, each a vertical stack of oval modules connected by solid downward-pointing arrows.
        *   **Pattern 1: Naive RAG**
            *   Modules: "Retrieve" (orange oval) → "Read" (orange oval)
            *   Title: "Naive RAG"
        *   **Pattern 2: Advanced RAG**
            *   Modules: "Rewrite" (blue oval) → "Retrieve" (orange oval) → "Rerank" (blue oval) → "Read" (orange oval)
            *   Title: "Advanced RAG"
        *   **Pattern 3: DSP**
            *   Modules: "Demonstrate" (blue oval) → "Search" (blue oval) → "Predict" (blue oval)
            *   Title: "DSP"
            *   Reference: "[khattab et al, 2022]"
        *   **Pattern 4: ITER-RETGEN**
            *   Modules: "Retrieve" (orange oval) → "Read" (orange oval) → "Retrieve" (orange oval) → "Read" (orange oval)
            *   An arc arrow indicates an iterative loop from the bottom "Read" back up to the top "Retrieve".
            *   Title: "ITER-RETGEN"
            *   Reference: "[shao et al, 2023]"
...
<!-- IMAGE_START: _pictures_3.png | label=Figure 4 | caption=RAG compared with other model optimization methods in the aspects of 'External Knowledge Required' and 'Model Adaption Required'. Prompt Engineering requires low modifications to the model and external knowledge, focusing on harnessing the capabilities of LLMs themselves. Fine-tuning, on the other hand, involves further training the model. In the early stages of RAG (Naive RAG), there is a low demand for model modifications. As research progresses, Modular RAG has become more integrated with fine-tuning techniques. -->
**Figure 4** (Fig. IV): RAG compared with other model optimization methods in the aspects of 'External Knowledge Required' and 'Model Adaption Required'. Prompt Engineering requires low modifications to the model and external knowledge, focusing on harnessing the capabilities of LLMs themselves. Fine-tuning, on the other hand, involves further training the model. In the early stages of RAG (Naive RAG), there is a low demand for model modifications. As research progresses, Modular RAG has become more integrated with fine-tuning techniques.
This image is a 2D conceptual diagram illustrating different methods for enhancing language models, categorized by two main axes: "External Knowledge Required" (Y-axis) and "Model Adaptation Required" (X-axis). Both axes range from "Low" to "High".

**Axis Labels:**
*   **Y-axis:** "External Knowledge Required"
    *   Bottom: "Low"
    *   Top: "High"
*   **X-axis:** "Model Adaptation Required"
    *   Left: "Low"
    *   Right: "High"

**Main Conceptual Regions and Their Locations:**
The diagram features a curved, dashed blue line that divides the space into roughly four quadrants, emphasizing different solution spaces. This line represents a boundary or transition zone between different approaches.

1.  **Prompt Engineering:**
    *   This region primarily occupies the **lower-left quadrant** (Low External Knowledge Required, Low Model Adaptation Required) and extends upwards along the "Low Model Adaptation Required" side.
    *   The term "Prompt Engineering" is written in bold blue text, curving diagonally upwards.

2.  **RAG (Retrieval-Augmented Generation):**
    *   This region is an orange-shaded, amorphous blob located in the **upper-middle-left quadrant** (High External Knowledge Required, relatively Low to moderate Model Adaptation Required).
    *   The term "RAG" is written in bold orange text within this blob.

3.  **Fine-tuning:**
    *   This region is a blue-shaded, amorphous blob located in the **lower-right quadrant** (Low to moderate External Knowledge Required, High Model Adaptation Required).
    *   The term "Fine-tuning" is written in bold blue text within this blob.

4.  **All of the above:**
    *   This region is an orange-shaded, amorphous blob located in the **upper-right quadrant** (High External Knowledge Required, High Model Adaptation Required).
    *   The phrase "All of the above" is written in bold black text within this blob.

**Specific Methods and Their Descriptions:**

**Under "Prompt Engineering" (primarily low Model Adaptation):**
These methods are arranged vertically, indicating an increasing "External Knowledge Required."

*   **Standard Prompt:**
    *   Located at the very bottom-left, indicating "Low External Knowledge Required" and "Low Model Adaptation Required."
    *   Represented by an oval-shaped box.
*   **Few-shot Prompt:**
    *   Located above "Standard Prompt."
    *   Represented by an oval-shaped box.
    *   A dashed blue arrow points from "Standard Prompt" to "Few-shot Prompt," indicating progression.
*   **XoT Prompt e.g. CoT, ToT:**
    *   Located above "Few-shot Prompt."
    *   Represented by an oval-shaped box.
    *   A solid blue arrow points from "Few-shot Prompt" to "XoT Prompt," indicating progression. (Note: "XoT" likely refers to "Chain-of-Thought" (CoT) or "Tree-of-Thought" (ToT) prompting.)

**Leading towards "RAG" (high External Knowledge Required, low to moderate Model Adaptation):**
These methods build on prompt engineering concepts and move towards integrating external knowledge.

*   **Naive RAG:**
    *   Located above "XoT Prompt."
    *   Represented by an oval-shaped box with the description: "Add relevant contextual paragraphs."
    *   A solid blue arrow points from "XoT Prompt" to "Naive RAG," indicating progression.
*   **Advanced RAG:**
    *   Located above "Naive RAG."
    *   Represented by an oval-shaped box with the description: "Index/pre-retrieval/post-retrieval optimization."
    *   A solid blue arrow points from "Naive RAG" to "Advanced RAG," indicating progression.
*   **Modular RAG:**
    *   Located at the top-left, above "Advanced RAG."
    *   Represented by an oval-shaped box with the description: "Organic combination of multiple modules."
    *   A solid blue arrow points from "Advanced RAG" to "Modular RAG," indicating progression.
    *   This box is linked by a dashed blue arrow to the "RAG" blob.

**Leading towards "Fine-tuning" (high Model Adaptation Required, low to moderate External Knowledge Required):**

*   **Generator Fine-tuning:**
    *   Located to the right of the central curved line, squarely within the "Fine-tuning" blob's influence (high Model Adaptation Required, moderate External Knowledge Required).
    *   Represented by an oval-shaped box.
    *   A dashed blue line connects the "Fine-tuning" blob to this box.
*   **Collaborative Fine-tuning:**
    *   Located above "Generator Fine-tuning," within the "All of the above" blob's influence.
    *   Represented by an oval-shaped box.
    *   A dashed blue line connects the "All of the above" blob to this box.
*   **Retriever Fine-tuning:**
    *   Located above "Collaborative Fine-tuning," also within the "All of the above" blob's influence.
    *   Represented by an oval-shaped box.
    *   A dashed blue line connects the "All of the above" blob to this box.

**Connections and Relationships:**

*   **Progression within Prompt Engineering and RAG:** Solid blue arrows show an evolutionary path from "Standard Prompt" up through "Modular RAG," indicating increasing sophistication and external knowledge requirements while keeping model adaptation low.
*   **Transition from Prompt Engineering to RAG:** Dashed blue lines connect "Few-shot Prompt" and "Naive RAG" to the "RAG" blob, suggesting these methods are stepping stones or components of RAG.
*   **Transition from Prompt Engineering to Fine-tuning:** A dashed blue line connects "Standard Prompt" to the "Fine-tuning" blob, implying that basic prompting might evolve directly into fine-tuning for model adaptation.
*   **Interplay between RAG and Fine-tuning:** Dashed blue arrows from "Modular RAG" curve rightwards towards the "All of the above" region, suggesting that advanced RAG can be combined with high model adaptation. Similarly, the "All of the above" region is linked to the fine-tuning methods (Retriever, Collaborative, Generator Fine-tuning). The "RAG" and "Fine-tuning" blobs are separated by a curved boundary, indicating distinct but potentially combinable approaches.
*   **The central dashed blue curve:** This curve visually separates the space into approaches that primarily rely on "Prompt Engineering / RAG" (lower Model Adaptation) from those that primarily rely on "Fine-tuning" (higher Model Adaptation). It also suggests a blending of approaches ("All of the above") in the top-right.

The image effectively uses its two dimensions and visual cues (arrows, shaded regions, box labels) to map out a landscape of techniques for improving language model performance.
<!-- IMAGE_END -->
...
<!-- IMAGE_START: _pictures_4.png | label=Figure 5 | caption=In addition to the most common once retrieval, RAG also includes three types of retrieval augmentation processes. (left) Iterative retrieval involves alternating between retrieval and generation, allowing for richer and more targeted context from the knowledge base at each step. (Middle) Recursive retrieval involves gradually refining the user query and breaking down the problem into sub-problems, then continuously solving complex problems through retrieval and generation. (Right) Adaptive retrieval focuses on enabling the RAG system to autonomously determine whether external knowledge retrieval is necessary and when to stop retrieval and generation, often utilizing LLM-generated special tokens for control. -->
**Figure 5** (Fig. V): In addition to the most common once retrieval, RAG also includes three types of retrieval augmentation processes. (left) Iterative retrieval involves alternating between retrieval and generation, allowing for richer and more targeted context from the knowledge base at each step. (Middle) Recursive retrieval involves gradually refining the user query and breaking down the problem into sub-problems, then continuously solving complex problems through retrieval and generation. (Right) Adaptive retrieval focuses on enabling the RAG system to autonomously determine whether external knowledge retrieval is necessary and when to stop retrieval and generation, often utilizing LLM-generated special tokens for control.
This image presents three distinct process diagrams side-by-side, each illustrating a different approach to a task, identified as "ITERATIVE," "RECURSIVE," and "ADAPTIVE." Each diagram is enclosed within a rounded rectangle with a light purple border and a light beige background.

---

### **1. ITERATIVE Diagram**

This diagram is titled "ITERATIVE" with the subtitle "Provide more context information." It outlines a sequential process with a feedback loop for repeated execution.

*   **Start Node:** A green rounded rectangle labeled "Query" at the top.
*   **Process Flow (Solid Arrows):**
    *   A solid blue arrow points down from "Query" to a grey rounded rectangle labeled "Retrieve."
    *   A solid blue arrow points down from "Retrieve" to a grey rounded rectangle labeled "Generate."
    *   A solid blue arrow points down from "Generate" to an orange diamond shape labeled "Judge."
*   **Decision and Loop:**
    *   From the "Judge" node, a dashed blue arrow loops back upwards to the "Retrieve" node.
    *   Alongside this dashed loop, a light grey rounded rectangle contains the text "Iterate N Times," indicating the condition for the loop.
*   **End Condition and Output:**
    *   A solid blue arrow points down from the "Judge" node to a light grey rounded rectangle with the text "Max Times / Threshold."
    *   This then leads to the final output: a green rounded rectangle labeled "Response" at the bottom.

---

### **2. RECURSIVE Diagram**

This diagram is titled "RECURSIVE" with the subtitle "Break down complex problems step by step." It illustrates a process involving query transformation and decomposition.
...
*   **Process Flow (Solid Arrows):**
    ...
    *   A solid blue arrow points down from "Retrieve" to a grey rounded rectangle labeled "Generate."
    *   A solid blue arrow points down from "Generate" to an orange diamond shape labeled "Judge."
*   **Decision and Recursive Step:**
    *   From the "Judge" node, a dashed blue arrow points to the right towards a grey rounded rectangle labeled "Query Transformation / Decomposition."
    *   From "Query Transformation / Decomposition," a dashed blue arrow loops back upwards to the "Retrieve" node, indicating a recursive call or re-evaluation.
*   **End Condition and Output:**
    *   A solid blue arrow points down from the "Judge" node to a light grey rounded rectangle with the text "Max Depth (Tree) / Threshold."
    *   This then leads to the final output: a green rounded rectangle labeled "Response" at the bottom.
...
This diagram is titled "ADAPTIVE" with the subtitle "Flexible and active control of retrieval and generation." It presents a more complex process with multiple decision points and feedback mechanisms.

*   **Start Node:** A green rounded rectangle labeled "Query" at the top.
*   **Initial Decision Point:**
    *   A solid blue arrow points down from "Query" to an upper orange diamond shape labeled "Judge."
*   **Branching from Upper Judge:**
    *   A solid blue arrow points down from the upper "Judge" to a grey rounded rectangle labeled "Retrieve."
    *   A dashed blue arrow points right from the upper "Judge" to a light grey rounded rectangle containing the text "Retrieve On Demand."
    *   A dashed blue arrow points down and left from the upper "Judge" to a grey rounded rectangle labeled "Generate" (referred to as "left Generate").
    *   A dashed blue arrow points down and right from the upper "Judge" to another grey rounded rectangle labeled "Generate" (referred to as "right Generate").
*   **Retrieve Flow:**
    *   A solid blue arrow points down from "Retrieve" to the "left Generate" node.
*   **Feedback Loops and Transformations:**
    *   From the "right Generate" node, a dashed blue arrow loops back up to the upper "Judge" node.
    *   From a grey rounded rectangle labeled "Query Transformation / Decomposition," a dashed blue arrow loops back up to the "Retrieve" node.
*   **Second Decision Point:**
    *   A solid blue arrow points down from the "left Generate" node to a lower orange diamond shape labeled "Judge."
*   **Branching from Lower Judge:**
    *   A dashed blue arrow points right from the lower "Judge" node to the "Query Transformation / Decomposition" node.
*   **End Condition and Output:**
    *   A solid blue arrow points down from the lower "Judge" node to a light grey rounded rectangle with the text "Generate Special Token / Threshold."
    *   This then leads to the final output: a green rounded rectangle labeled "Response" at the bottom. A small dashed blue arrow also leads from the "Generate Special Token / Threshold" box to the "Response" node.

---

**Summary of Shapes and Colors:**
*   **Start/End Nodes (Query, Response):** Green rounded rectangles.
*   **Process Steps (Retrieve, Generate, Query Transformation / Decomposition):** Grey rounded rectangles.
*   **Decision Points (Judge):** Orange diamond shapes.
*   **Labels/Conditions (Iterate N Times, Max Times / Threshold, Max Depth (Tree) / Threshold, Retrieve On Demand, Generate Special Token / Threshold):** Light grey rounded rectangles.
*   **Main Flow Arrows:** Solid blue arrows.
*   **Feedback/Loop/Conditional Arrows:** Dashed blue arrows.
<!-- IMAGE_END -->
...
<!-- IMAGE_START: _pictures_5.png | label=Figure 6 | caption=Summary of RAG ecosystem -->
**Figure 6** (Fig. VI): Summary of RAG ecosystem
This image is a detailed concept map or organizational chart outlining various aspects of Retrieval Augmented Generation (RAG). It is structured into six main sections, each with a distinct heading, and further subdivided into categories and specific data points. The overall aesthetic is clean and modern, using light blue, light purple, and light yellow rounded rectangles and ovals to group information.

Here is a full description, extracting all visible data and labels:

**Overall Structure:**
The diagram is organized into two main vertical columns. The left column focuses on the RAG process itself, including its ecosystem, paradigm, techniques, and key issues. The right column focuses on the future prospect and evaluation of RAG. Each major section title is prefixed with a right-pointing arrow icon and displayed in blue text.

---

**Left Column (RAG Fundamentals and Techniques):**

**1. RAG Ecosystem**
*   **Title:** ▸ RAG Ecosystem
*   **Sub-sections (separated by a vertical line):**
    *   **Downstream Tasks:**
        *   Dialogue (in a rounded white rectangle with a thin border)
        *   Question answering (in a rounded white rectangle with a thin border)
        *   Summarization (in a rounded white rectangle with a thin border)
        *   Fact verification (in a rounded white rectangle with a thin border)
    *   **Technology Stacks:**
        *   Langchain (in a rounded white rectangle with a thin border)
        *   LlamaIndex (in a rounded white rectangle with a thin border)
        *   FlowiseAI (in a rounded white rectangle with a thin border)
        *   AutoGen (in a rounded white rectangle with a thin border)

**2. The RAG Paradigm**
*   This section is enclosed within a large light purple rounded rectangle, which is itself part of an even larger light blue rounded rectangle that spans most of the left column's height.
*   **Title:** ▸ The RAG Paradigm
*   **Content:**
    *   Naive RAG (in a rounded white rectangle with a thin border)
    *   Advanced RAG (in a rounded white rectangle with a thin border)
    *   Modular RAG (text label)
    *   An arrow points from "Naive RAG" to "Advanced RAG".
    *   An arrow points from "Advanced RAG" to "Modular RAG".

**3. Techniques for Better RAG**
*   This section is located below "The RAG Paradigm" and is enclosed within a light yellow rounded rectangle.
*   **Title:** ▸ Techniques for Better RAG
*   **Content (all in rounded white rectangles with thin borders):**
    *   Chunk Optimization
    *   Iterative Retrieval
    *   Retriever Fine-tuning
    *   Query Transformation
...
**4. Key Issues of RAG**
*   This section is nested within "Techniques for Better RAG" and is enclosed within a lighter purple rounded rectangle with a horizontal gradient, transitioning from a light pink on the left to a light blue/purple on the right.
*   **Title:** ▸ Key Issues of RAG
*   **Content (all in oval shapes with a horizontal gradient matching the background):**
    *   What to retrieve (pinkish oval)
    *   When to retrieve (light purple oval)
    *   How to use Retrieval (purple oval)

---

**Right Column (RAG Prospects and Evaluation):**

**5. RAG Prospect**
*   **Title:** ▸ RAG Prospect
*   **Sub-sections (separated by vertical lines):**
    *   **Challenges:**
        *   RAG in Long Context Length (in a rounded white rectangle with a thin border)
        *   Hybrid (in a rounded white rectangle with a thin border)
        *   Robustness (in a rounded white rectangle with a thin border)
        *   Scaling-laws for RAG (in a rounded white rectangle with a thin border)
        *   Production-ready RAG (in a rounded white rectangle with a thin border)
    *   **Modality Extension:**
        *   Image (in a rounded white rectangle with a thin border)
        *   Audio (in a rounded white rectangle with a thin border)
        *   Video (in a rounded white rectangle with a thin border)
        *   Code (in a rounded white rectangle with a thin border)
    *   **Ecosystem:**
        *   Customization (in a rounded white rectangle with a thin border)
        *   Simplification (in a rounded white rectangle with a thin border)
        *   Specialization (in a rounded white rectangle with a thin border)

**6. Evaluation of RAG**
*   This section is located below "RAG Prospect" and is enclosed within the same large light blue rounded rectangle that contains the "RAG Prospect" section.
*   **Title:** ▸ Evaluation of RAG
*   **Sub-sections:**
    *   **Evaluation Target:**
        *   Retrieval Quality (in a rounded white rectangle with a thin border)
        *   Generation Quality (in a rounded white rectangle with a thin border)
    *   **Evaluation Aspects:**
        *   Answer Relevance (in a rounded white rectangle with a thin border)
        *   Noise Robustness (in a rounded white rectangle with a thin border)
        *   Context Relevance (in a rounded white rectangle with a thin border)
        *   Negation Rejection (in a rounded white rectangle with a thin border)
        *   Answer Faithfulness (in a rounded white rectangle with a thin border)
        *   Information Integration (in a rounded white rectangle with a thin border)
        *   Counterfactual Robustness (in a rounded white rectangle with a thin border)
    *   **Evaluation Framework:**
        *   **Benchmarks:**
            *   CRUD (in a rounded white rectangle with a thin border)
            *   RGB (in a rounded white rectangle with a thin border)
            *   RECALL (in a rounded white rectangle with a thin border)
        *   **Tools:**
            *   TruLens (in a rounded white rectangle with a thin border)
            *   RAGAS (in a rounded white rectangle with a thin border)
            *   ARES (in a rounded white rectangle with a thin border)

All text is in a clear, dark sans-serif font. The background of the entire image is white.
<!-- IMAGE_END -->
